          ...
          <div class="flex-1 overflow-y-auto p-6">
            ...
            <div v-if="canManageUsers" class="flex gap-1 bg-slate-100 rounded-xl p-1 mb-5">
              ...
              <button
                ...
                :class="['flex-1 flex items-center justify-center gap-1 py-2 text-xs sm:text-sm font-semibold rounded-lg transition', activeTab === 'sources' ? 'bg-white text-primary shadow-sm' : 'text-slate-500 hover:text-slate-700']"
              >
            ...
            <template v-else-if="activeTab === 'sources'">
              <div class="space-y-6">
                ...
                <!-- Header -->
                <div>
                  <h3 class="text-lg font-bold text-slate-800 mb-1">Source Options</h3>
                  <p class="text-sm text-slate-500">Configure the source options shown in lead forms.</p>
                </div>

                <!-- Add new source -->
                <div>
                  <label class="block text-sm font-bold text-slate-700 mb-2">Add New Source</label>
                  <div class="flex gap-2">
                    <input
                      v-model="newSource"
                ...
                <div>
                  <div class="flex items-center justify-between mb-3">
                    <label class="text-sm font-bold text-slate-700">Current Options</label>
                    <span class="text-xs font-semibold text-slate-400 bg-slate-100 px-2 py-1 rounded">{{ sourcesList.length }} total</span>
                  </div>
                  <div class="bg-slate-50 rounded-xl border border-slate-200 p-3 max-h-96 overflow-y-auto">
                    <div v-if="sourcesList.length === 0" class="text-center py-12 text-slate-400">
                      <i class="ph-bold ph-funnel text-4xl mb-2 block"></i>
                      <p class="text-sm font-medium">No sources configured yet</p>
                      <p class="text-xs mt-1">Add your first source option above</p>
                    </div>
                    <div v-else class="space-y-2">
                      <div
                ...
                <div class="flex items-center gap-3 justify-end pt-4 border-t border-slate-200">
                  <Transition name="form-slide">
                    <span v-if="sourcesSaved" class="text-sm text-green-600 font-semibold flex items-center gap-1.5">
                      <i class="ph-bold ph-check-circle text-lg"></i> Saved!
                    </span>
                  </Transition>
                  <button
                    ...
                    class="flex items-center gap-2 bg-primary text-white text-sm font-semibold px-6 py-2.5 rounded-lg hover:bg-indigo-700 disabled:opacity-60 transition shadow-sm"
                  >
                    <i v-if="sourcesSaving" class="ph-bold ph-spinner-gap animate-spin text-lg"></i>
                    <i v-else class="ph-bold ph-floppy-disk text-lg"></i>
                    {{ sourcesSaving ? 'Saving…' : 'Save Changes' }}
                  </button>
                </div>
...
const activeTab = ref<'users' | 'appearance' | 'interests' | 'sources'>('users')
...
// Sources management
const sourcesList = ref<string[]>([...(appStore.sourcesList || [])])
const newSource = ref('')
...
function addSource() {
  ...
  if (source && !sourcesList.value.includes(source)) {
    sourcesList.value.push(source)
    newSource.value = ''
  }
}

function removeSource(idx: number) {
  sourcesList.value.splice(idx, 1)
}

async function saveSources() {
  sourcesSaving.value = true
  try {
    await appStore.saveSourcesList(sourcesList.value)
    sourcesSaved.value = true
    setTimeout(() => { sourcesSaved.value = false }, 2000)
  } catch (err) {
...
watch(() => props.isOpen, (open) => {
  if (open) {
    ...
    sourcesList.value = [...(appStore.sourcesList || [])]
  } else {
    showForm.value = false
    resetForm()
    confirmDeleteId.value = null
